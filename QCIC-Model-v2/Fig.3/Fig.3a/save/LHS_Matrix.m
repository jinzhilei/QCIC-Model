...
S(:,8)=Beta_Matrix(0.01,0.06,2,2,sample); % beta_X3

%% environmental carrying capacity
S(:,9)=Beta_Matrix(1.5e9,4e9,5,3,sample); % K1
S(:,10)=Beta_Matrix(1e9,3e9,7,4,sample);  % K2
S(:,11)=Beta_Matrix(9e8,2e9,4,7,sample);  % K3

%% Tumor apoptosis
S(:,12)=Beta_Matrix(0.02,0.04,3,7,sample); % d_X1
S(:,13)=Beta_Matrix(0.015,0.03,4,6,sample);  % d_X2
S(:,14)=Beta_Matrix(0.005,0.01,3,2,sample); % d_X3

%% chemotherapy

S(:,15)=Beta_Matrix(0.2,0.9,1,1,sample); % rho_X3X1
S(:,16)=Beta_Matrix(0.2,0.9,1,1,sample); % rho_X3X2
S(:,17)=Beta_Matrix(0.01,0.1,5,5,sample); % sigma_X1
S(:,18)=Beta_Matrix(0.01,0.1,5,5,sample); % sigma_X2
