function y = QCIC(t,x,K_TN4,K_TN8,rho_DD0,phi_P,phi_L,beta_X1,beta_X2,beta_X3,K1,K2,K3,d_X1,d_X2,d_X3,rho_X3X1,rho_X3X2,sigma_X1,sigma_X2,varepsilon_TX1,varepsilon_TX2,varepsilon_TX3)
...
% PD-1 in TME
P_C = phi_P * (T1_C + T2_C + T17_C +  Tfh_C + T8_C + par.varepsilon_NK * NK_C) ;
...
proliferation_TN4_A = par.beta_TN4 * TN4_A * (1 - (TN4_A / K_TN4));
...
proliferation_TN8_A = par.beta_TN8 * TN8_A * (1 - (TN8_A / K_TN8));
...
% DSTC in TME
proliferation_X1_C = (beta_X1 / (1 + varepsilon_TX1 * Targ_C)) * (1 - (par.a11 * X1_C + par.a12 * X2_C + par.a13 * X3_C )/K1) * X1_C;
transformation_X3X1 = rho_X3X1 * ( Chem_C^par.n_c / (par.K_Chem^par.n_c + Chem_C^par.n_c) ) * X1_C;
apoptosis_X1_C = d_X1 * X1_C;
immune_X1_C = (par.eta_X1T1 * T1_C + par.eta_X1T8 * T8_C + par.eta_X1NK * NK_C) * (1 - par.lambda_PL * (PL_C / (par.K_PL + PL_C))) * X1_C;
chemotherapy_X1_C = sigma_X1 * (1 - exp(-par.varepsilon_CX1 * Chem_C)) * X1_C;

% DRTC in TME
proliferation_X2_C = (beta_X2 / (1 + varepsilon_TX2 * Targ_C)) * (1 - (par.a21 * X1_C + par.a22 * X2_C + par.a23 * X3_C )/K2) * X2_C;
transformation_X3X2 = rho_X3X2 * ( Chem_C^par.n_c / (par.K_Chem^par.n_c + Chem_C^par.n_c) ) * X2_C;
apoptosis_X2_C = d_X2 * X2_C;
immune_X2_C = (par.eta_X2T1 * T1_C + par.eta_X2T8 * T8_C + par.eta_X2NK * NK_C) * (1 - par.lambda_PL * (PL_C / (par.K_PL + PL_C))) * X2_C;
chemotherapy_X2_C = sigma_X2 * (1 - exp(-par.varepsilon_CX2 * Chem_C)) * X2_C;

% DPTC in TME
proliferation_X3_C = (beta_X3 / (1 + varepsilon_TX3 * Targ_C)) * (1 - (par.a31 * X1_C + par.a32 * X2_C + par.a33 * X3_C )/K3) * X3_C;
apoptosis_X3_C = d_X3 * X3_C;
immune_X3_C = (par.eta_X3T1 * T1_C + par.eta_X3T8 * T8_C + par.eta_X3NK * NK_C) * (1 - par.lambda_PL * (PL_C / (par.K_PL + PL_C))) * X3_C;
chemotherapy_X3_C = par.sigma_X3 * (1 - exp(-par.varepsilon_CX3 * Chem_C)) * X3_C;

% TAA in TME
TA_C = (1 / par.d_TA) * ((par.kappa_1 * apoptosis_X1_C + par.kappa_1 * apoptosis_X2_C + par.kappa_1 * apoptosis_X3_C) + (par.kappa_2 * immune_X1_C + par.kappa_2 * immune_X2_C + par.kappa_3 * immune_X3_C) + (par.kappa_3 * chemotherapy_X1_C + par.kappa_3 * chemotherapy_X2_C + par.kappa_3 * chemotherapy_X3_C)) ;
...
% D0 in TME
transformation_DD0 = rho_DD0 * (TA_C^par.n / (par.K_TA^par.n + TA_C^par.n )) * D0_C;
...
y(34) = proliferation_X3_C + transformation_X3X1 + transformation_X3X1 - apoptosis_X3_C - immune_X3_C  - chemotherapy_X3_C ;
...
Targ = drugs_bevacizumab(par.k_in_Targ,t);

Chem = drugs_TAS102(par.k_in_Chem,t);

y(54) = Targ + par.k_21_Trag * Targ_P - par.k_12_Trag * Targ_C - par.k_10_Trag * Targ_C ;
y(55) = par.k_12_Trag * Targ_C - par.k_21_Trag * Targ_P ;

y(56) = Chem + par.k_21_Chem * Chem_P - par.k_12_Chem * Chem_C - par.k_10_Chem * Chem_C ;
y(57) = par.k_12_Chem * Chem_C - par.k_21_Chem * Chem_P ;
